...
name: notion-digest
description: >-
  Reads recently edited pages from the notion MCP server (read-only) and writes what it
...
Read notion, write a doc, open a PR. Nothing else.

Runs unattended. Never prompt for confirmation and never write to notion.

## Inputs

Supplied by the launcher via `.cloud-launch/inputs.json`. All optional.

- **`{{DIGEST_FILE}}`** -- repo-relative digest path. Default `docs/notion-digest.md`.
- **`{{LIMIT}}`** -- maximum items to list. Default `20`.
- **`{{BRANCH_PREFIX}}`** -- branch name prefix. Default `notion-digest`.

The run must provide the **notion MCP server**, registered on this workspace
and reachable as `notion` through the Tessl MCP gateway. GitHub credentials
...
assume a prefix: the gateway may expose them as `mcp__notion__*`,
`mcp__drew-test_notion__*`, or another form. Match on the tool names you
actually see.

1. Search or list recently edited pages, newest first, at most `{{LIMIT}}`.
2. For each page keep its title, URL, and last-edited timestamp.

If no notion MCP tool is available, stop and report that, and say which tool
...
git add "{{DIGEST_FILE}}"
git commit -m "docs: refresh notion digest"
git push -u origin "$BRANCH"
gh pr create --title "docs: refresh notion digest" \
  --body "Automated digest read from the notion MCP server and written to \`{{DIGEST_FILE}}\`."
```
